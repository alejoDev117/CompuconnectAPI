...
public final class ModificarReservaValidation implements Validation<ReservaDTO> {

    private ModificarReservaValidation() {
        super();
    }

    public static Result validate(final ReservaDTO data) {
        return new ModificarReservaValidation().execute(data);
    }
    ...
    public Result execute(final ReservaDTO data) {
        ...
        if (UtilObject.isNull(data)) {
            result.addMessage("No es posible modificar la reserva con los datos vacíos");
        } else {
