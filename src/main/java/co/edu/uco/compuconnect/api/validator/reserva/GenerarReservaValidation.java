...
public final class GenerarReservaValidation implements Validation<ReservaDTO> {

    private GenerarReservaValidation() {
        super();
    }

    public static Result validate(final ReservaDTO data) {
        return new GenerarReservaValidation().execute(data);
    }
    ...
    public Result execute(final ReservaDTO data) {
        ...
        if (UtilObject.isNull(data)) {
            result.addMessage("No es posible generar una reserva con los datos vacíos");
        } else {
