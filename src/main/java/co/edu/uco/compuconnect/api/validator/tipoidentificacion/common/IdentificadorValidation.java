...
public class IdentificadorValidation implements Validation<UUID>{
	...
	public final Result execute(UUID data) {
		...
		if(UtilObject.isNull(data)) {
			result.addMessage("No es posible continuar con el identificador del tipo identificacion vacio");
		}
		
		else if(UtilUUID.isDefault(data)) {
			result.addMessage("No es posible tener el identificador por defecto del tipo identificacion vacio");
		}
