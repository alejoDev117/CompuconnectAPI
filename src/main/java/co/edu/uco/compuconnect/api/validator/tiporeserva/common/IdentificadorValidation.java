...
public class IdentificadorValidation implements Validation<UUID>{
	...
	public final Result execute(UUID data) {
		...
		if(UtilObject.isNull(data)) {
			result.addMessage("No es posible continuar con el identificador del tipo reserva vacio");
		}
		
		else if(UtilUUID.isDefault(data)) {
			result.addMessage("No es posible tener el identificador por defecto del tipo reserva vacio");
		}
