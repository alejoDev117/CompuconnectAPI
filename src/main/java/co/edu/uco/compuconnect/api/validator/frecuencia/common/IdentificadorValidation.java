...
public class IdentificadorValidation implements Validation<UUID>{
	...
	public final Result execute(UUID data) {
		...
		if(UtilObject.isNull(data)) {
			result.addMessage("No es posible continuar con el identificador de la frecuencia vacio");
		}
		
		else if(UtilUUID.isDefault(data)) {
			result.addMessage("No es posible tener el identificador por defecto de la frecuencia");
		}
