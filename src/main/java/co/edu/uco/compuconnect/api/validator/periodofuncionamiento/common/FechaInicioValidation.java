...
public class FechaInicioValidation implements Validation<Date> {
    ...
    public Result execute(Date data) {
        ...
        if (UtilObject.isNull(data)) {
            result.addMessage("La fecha de inicio del periodo de funcionamiento no puede estar vacía");
        } else {
            Date currentDate = new Date();
            if (data.before(currentDate)) {
                result.addMessage("La fecha de inicio del periodo de funcionamiento no puede ser anterior a la fecha actual");
            }
        }
